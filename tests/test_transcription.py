...
class TestTranscriptionService:
    ...
    def test_format_transcript_email(self):
        ...
        with patch.object(self.transcription_service.client.chat.completions, 'create') as mock_create:
            mock_response = Mock()
            mock_response.choices = [Mock()]
            mock_response.choices[0].message.content = "Subject: Meeting Tomorrow\n\nHi,\n\nThis is regarding the meeting tomorrow.\n\nBest regards"
            mock_create.return_value = mock_response
            
            result = self.transcription_service.format_transcript(raw_text, TranscriptFormat.EMAIL)
    ...
    def test_format_transcript_notes(self):
        ...
        with patch.object(self.transcription_service.client.chat.completions, 'create') as mock_create:
            mock_response = Mock()
            mock_response.choices = [Mock()]
            mock_response.choices[0].message.content = "• Buy groceries:\n  - Milk\n  - Bread\n  - Eggs\n• Call the dentist"
            mock_create.return_value = mock_response
            
            result = self.transcription_service.format_transcript(raw_text, TranscriptFormat.NOTES)
    ...
    def test_format_transcript_raw(self):
        ...
        result = self.transcription_service.format_transcript(raw_text, TranscriptFormat.RAW)
    ...
    def test_generate_summary_long_text(self):
        ...
        long_text = "This is a very long message that exceeds the maximum length and needs to be summarized. " * 5
    ...
    async def test_transcribe_audio_success(self):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with patch.object(self.transcription_service.client.audio.transcriptions, 'create') as mock_transcribe:
